...
if ($Restore) {
  $backup = $null
  if ($BackupPath -and (Test-Path -LiteralPath $BackupPath)) { $backup = Get-Item -LiteralPath $BackupPath }
  if (-not $backup) { $backup = Get-LatestBackup $worker }
...
if ($content.Contains($marker)) {
  ...
  if ($PassThru) { return [pscustomobject]@{ Path = $worker; Backup = $null; OriginalHash = $null; PatchedHash = (Get-Hash $worker); AlreadyPatched = $true } }
  exit 0
}
...
foreach ($anchor in $required) {
  if (-not $content.Contains($anchor)) { throw "worker.cjs 结构与已验证版本不一致，未执行修改。缺少锚点：$anchor" }
}
$originalHash = Get-Hash $worker
...
$updated = $updated.Replace('currentThreadId: () => getCurrentThreadId(),', "currentThreadId: () => getCurrentThreadId(),`r`n`t`tforegroundWindow: () => getForegroundWindow(),")
$updated = $updated.Replace('show: () => method(dialog, SLOT_SHOW, protoShow)(null),', 'show: (owner = null) => method(dialog, SLOT_SHOW, protoShow)(owner),')
$updated = $updated.Replace('const shown = dialog.show();', 'const shown = dialog.show(bindings.foregroundWindow());')
$verification = @(
  $ownerBinding,
...
foreach ($item in $verification) { if (-not $updated.Contains($item)) { throw "补丁生成后校验失败：$item" } }
...
if ($PassThru) { return [pscustomobject]@{ Path = $worker; Backup = $backupPath; OriginalHash = $originalHash; PatchedHash = $patchedHash; AlreadyPatched = $false } }
